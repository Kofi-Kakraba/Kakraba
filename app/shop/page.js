...
export default function ShopPage() {
  ...
  // Active Cart Tracking & Slide-Over Drawer Toggle States
  const [cart, setCart] = useState([]);
  const [isCartOpen, setIsCartOpen] = useState(false);
  const [appliedCoupon, setAppliedCoupon] = useState(null); 

  // Isolated local input counters indexed by Variant Key IDs
  const [localQuantities, setLocalQuantities] = useState({});

  // NEW: Tracks the exact text micro-interactions per variant ID ('idle' | 'adding' | 'added')
  ...
  // ====== UPDATED: Orchestrates the 'Adding' -> 'Added' -> 'Reset' State Machine Flow ======
  const handleAddItemToCartChannel = (product, variant, continuousQuantity) => {
    if (continuousQuantity <= 0) return;

    // Phase 1: Set state status to 'adding' to update button text & lock interactions
    setButtonStatuses(prev => ({ ...prev, [variant.id]: 'adding' }));

    // Phase 2: Wait 600ms to allow visual confirmation that processing has begun
    setTimeout(() => {
      setCart(prevCart => {
      ...
      // Reset card selection picker count back to 1 and change button text to 'added'
      setLocalQuantities(prev => ({ ...prev, [variant.id]: 1 }));
      setButtonStatuses(prev => ({ ...prev, [variant.id]: 'added' }));

      // Phase 3: Hold 'added' checkmark for 1.2 seconds, then smoothly reset back to idle
      setTimeout(() => {
    ...
    <div className="min-h-screen bg-stone-50 text-stone-800 antialiased font-sans pb-24 selection:bg-emerald-600 selection:text-white relative">
      ...
      <header className="bg-white/95 backdrop-blur-md border-b border-stone-200/60 py-4 px-6 sticky top-0 z-40 shadow-sm flex justify-between items-center h-20">
        <div className="flex items-center gap-2 h-full">
          <img src="/SPARKLE BEV. LOGO A No BG.png" alt="Logo" className="h-14 object-contain" />
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {products.map((product) => 
            product.product_variants?.map((variant) => {
              ...
              // Read current localized micro-state for this specific element key
              ...
              return (
                <div key={variant.id} className="bg-white border border-stone-200/60 rounded-[28px] p-5 flex flex-col justify-between space-y-4 shadow-sm hover:shadow-md transition-all">
                  ...
                  {/* SIMPLIFIED INLINE SIZE MOQ WARNING BAR */}
                  ...
                  {/* STEP QUANTITY PICKER ROW BAR SECTION */}
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    {/* ====== UPDATED DYNAMIC MULTI-STAGE ANIMATION BUTTON ====== */}
                    <button 
                      type="button" 
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                {cart.length > 0 && (
                  <form onSubmit={handleLaunchPaystackPaymentPortalGateway} className="space-y-4 font-mono text-xs pt-4 border-t border-stone-100">
                    ...
                    {/* SIMPLIFIED CART DRAWER MOQ DISPLAYS */}
                    <div className="bg-stone-950 p-4 border border-stone-800 rounded-2xl space-y-2 text-stone-400 text-[11px] shadow-2xl relative mt-4">
