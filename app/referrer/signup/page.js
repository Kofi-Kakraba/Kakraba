      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur-2xl border-2 border-white/50 rounded-[40px] p-8 md:p-10 shadow-2xl space-y-8 relative z-10">
        ...
        <div className="text-center space-y-4">
          ...
          <div className="inline-flex items-center gap-1.5 bg-stone-100 border border-stone-200 text-stone-600 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm">
            <Zap className="h-3 w-3 text-rose-500" /> Registry
          </div>
          <h2 className="text-3xl font-black text-stone-950 uppercase tracking-tighter leading-tight">Join The <br/>Squad.</h2>
          <p className="text-sm text-stone-500 font-bold max-w-xs mx-auto leading-relaxed">
            Apply to become an official Sparkle Ambassador and start earning on every drop.
          </p>
        </div>
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-5 text-left">
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" required value={usernameInput} onChange={(e) => setUsernameInput(e.target.value)} placeholder="e.g. SPK-YOURNAME" className="w-full bg-[#FDFBF7] border-2 border-stone-200 rounded-2xl pl-12 pr-4 py-3.5 text-stone-900 font-black uppercase tracking-widest outline-none focus:border-rose-500 transition-colors placeholder:text-stone-300 placeholder:font-medium" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="password" required value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder="••••••••" className="w-full bg-[#FDFBF7] border-2 border-stone-200 rounded-2xl pl-12 pr-4 py-3.5 text-stone-900 font-bold outline-none focus:border-rose-500 transition-colors placeholder:text-stone-300" />
            </div>
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-rose-600 hover:bg-rose-500 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 text-xs uppercase tracking-widest disabled:opacity-40 transition-all shadow-[0_8px_30px_rgb(225,29,72,0.3)] hover:-translate-y-1 mt-2">
            <span>{loading ? 'Processing...' : 'Submit Application'}</span> <ArrowRight className="h-4 w-4" />
          </button>
