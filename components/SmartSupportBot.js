...
export default function SmartSupportBot() {
  ...
  // Page-Specific FAQ Dictionaries
  const faqs = {
    shop: [
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[90] font-sans">
      ...
      {isOpen && (
        <div className="mb-4 w-[320px] bg-white border border-stone-200 rounded-[24px] shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="p-4 bg-[#FDFBF7] min-h-[250px] max-h-[350px] overflow-y-auto">
            {!activeQuestion ? (
              <div className="space-y-2">
                ...
                {currentFaqs.map((faq) => (
                  ...
                  >
                    <span className="text-xs font-bold text-stone-800">{faq.question}</span>
                    <ChevronRight className="h-4 w-4 text-stone-300 group-hover:text-emerald-500 shrink-0 ml-2" />
                  </button>
                ))}
              </div>
            ) : (
          ...
          <div className="p-4 bg-white border-t border-stone-100">
            ...
            <a 
              href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hey%20Sparkle!%20I%20need%20some%20help%20from%20the%20${pathname === '/' ? 'shop' : pathname}%20page.`}
              target="_blank"
      ...
      <button 
        ...
        className="w-14 h-14 bg-stone-950 hover:bg-stone-800 text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-105 transition-all ml-auto relative"
      >
